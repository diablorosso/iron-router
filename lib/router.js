...
IronRouter.prototype = {
  ...
  /**
   * check if the current route matches the routeNames route
   * and return a string (e.g. css class name) if so
   * Could be configured to match for subparts of the current
   * route so the tree path is also marked active
   *
   * @param {String} name The name of the route
   * @return {String}
   * @api public
  ...
  highlightIfActive: function (routeName, activeName, inactiveName) {
    /* let's get the route */
    ...
    /* and the parameters */
    var activeName = activeName || route.getFromData('activeName', 'active');
    var inactiveName = inactiveName || route.getFromData('inactiveName', '');

    /* sanitize the route before processing */
    if (routePath === void 0 || routePath === '') {
      routePath = '/';
    } else if (routePath.charAt(0) !== '/') {
      routePath = '/' + routePath;
    }
    var colonStart = routePath.indexOf(':');
    if (colonStart !== -1) {
      routePath = routePath.substring(0, colonStart);
    }
    if (routePath.length > 1 && routePath.charAt(routePath.length - 1) === '/') {
      routePath = routePath.slice(0, -1);
    }

    var currentPath = location.pathname;
    var paramsStart = currentPath.indexOf('?');
    if (paramsStart !== -1) {
      currentPath = currentPath.substring(0, paramsStart);
    }
    if (currentPath.length > 1 && currentPath.charAt(currentPath.length - 1) === '/') {
      currentPath = currentPath.slice(0, -1);
    }

    /* start splitting the parts */
    var routeParts = routePath.split('/');
    var currentParts = currentPath.split('/');

    /* find if on the correct path */
    var _i, _len;
    for (_i = 0, _len = routeParts.length; _i < _len; _i++) {
      if (routeParts[_i] !== currentParts[_i]) {
        return inactiveName;
      }
    }
    return activeName;
